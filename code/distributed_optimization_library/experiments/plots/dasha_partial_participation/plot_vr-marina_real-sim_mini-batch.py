...
def plot_results(dumps_paths, output_path, plot_functions, ignore_methods):
    ...
    for dumps_path in dumps_paths:
        ...
        for dump in dumps:
            ...
            # if (dump['params']['config']['algorithm_name'] == 'zero_marina_partial_participation_page' and 
            #         dump['params']['config']['algorithm_master_params']['number_of_samples'] != 10):
            #     continue
            dump['params']['config']['_algorithm_name'] = dump['params']['config']['algorithm_name']
            if ('zero_marina_partial_participation_page' in dump['params']['config']['algorithm_name']):
    ...
    representation_name = {'marina': 'MARINA',
                           ...
                           'zero_marina_partial_participation_page': 'DASHA-PP-PAGE',
    ...
    def get_representation_name(dump):
        ...
        if 'zero_marina_partial_participation_page' in algorithm_name:
            return representation_name['zero_marina_partial_participation_page'] + \
                r' ($s=$' + "{})".format(dump['params']['config']['algorithm_master_params']['number_of_samples'])
        else:
    ...
    best_experiments_to_markers = {'marina': {'marker': '<', 'color': 'red'},
                                   ...
                                   'zero_marina_partial_participation_page': {'marker': '>', 'color': 'orange'},
    ...
    def get_marker(algorithm_name):
        if 'zero_marina_partial_participation_page' in algorithm_name:
            return best_experiments_to_markers['zero_marina_partial_participation_page']
        else:
    ...
    for dump in dumps_loaded:
        ...
        #         'markersize': 25,
        #         'color': color}
        if get_representation_name(dump) in ignore_methods:
            continue
