...
def plot_results(dumps_paths, output_path, plot_functions, nodes):
    ...
    for dump in dumps_loaded:
        ...
        if not np.isnan(dump['stat']['norm_of_gradients']).any() and not np.isinf(dump['stat']['norm_of_gradients']).any():
            ...
            print(algorithm_name, dump['params']['config']['num_nodes'], dump['params']['config']['noise_lambda'], dump['params']['config']['algorithm_master_params']['gamma_multiply'], 'Diverged')
        index += 1
    top_show = 3
    ...
    for dump in dumps_loaded:
        ...
        if False:
            ...
            print(dump['stat']['max_bites_send_from_nodes'][:10])
            ...
            kwargs = {'label' :"{}: x{}".format(representation_name[dump['params']['config']['algorithm_name']],
                                                dump['params']['config']['algorithm_master_params']['gamma_multiply']),
                      'marker' : markers[processed_index[i][j]],
                      'markersize': 14,
                      'color': color}
